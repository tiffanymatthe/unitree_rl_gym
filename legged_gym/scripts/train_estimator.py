...
def train(args):
    ...
    buffer_actions = torch.zeros(
        num_steps + 1, num_processes, *action_shape, device="cpu"
    )
    ...
    estimator = load_model(model_path=None, num_obs=obs_dim-3, device=args.rl_device)

    optimizer = torch.optim.Adam(estimator.parameters(), lr=3e-4)

    obs = env.reset()[0]
    buffer_observations[0].copy_(obs.to("cpu"))
    buffer_actions[0].copy_(obs[:,0:3].to("cpu"))
    ...
    for epoch in range(NUM_EPOCHS):
        with torch.no_grad():
            ...
            for step in range(num_steps):
                ...
                buffer_actions[0].copy_(priv_obs[:,0:3].to("cpu"))
        ...
        for indices in shuffled_indices_batch:
            ...
            pred_actions = estimator.act_inference(observations_batch[:,3:].to("cuda:0")) # remove linear velocities
